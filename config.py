...
def _env_float(name: str, default: float) -> float:
    ...
    try:
        ...
        print(
            f"WARNING: invalid numeric config {name}={raw!r}; using default {default}.",
            file=sys.stderr,
        )
        return default
...
def _env_int(name: str, default: int) -> int:
    ...
    try:
        ...
        try:
            ...
            print(
                f"WARNING: invalid integer config {name}={raw!r}; using default {default}.",
                file=sys.stderr,
            )
            return default
...
# Live trading is deliberately locked behind an explicit multi-step opt-in.
# Keep this disabled while validating the strategy in demo/paper mode.
LIVE_TRADING_ENABLED = os.getenv("LIVE_TRADING_ENABLED", "false").lower() == "true"
LIVE_TRADING_CONFIRM = os.getenv("LIVE_TRADING_CONFIRM", "")
# Optional ISO date (YYYY-MM-DD). When set to today or a future date, live mode
# stays blocked even if LIVE_TRADING_ENABLED is true.
DEMO_ONLY_UNTIL = os.getenv("DEMO_ONLY_UNTIL", "")
...
REMOTE_POLL_INTERVAL_SECONDS = _env_float("REMOTE_POLL_INTERVAL_SECONDS", 2.0)
...
REMOTE_PNL_REPORT_INTERVAL_HOURS = _env_float("REMOTE_PNL_REPORT_INTERVAL_HOURS", 24.0)
REMOTE_PNL_REPORT_ON_START = os.getenv("REMOTE_PNL_REPORT_ON_START", "false").lower() == "true"
REMOTE_DEMO_BALANCE = _env_float("REMOTE_DEMO_BALANCE", 1000.0)
REMOTE_DEMO_POSITIONS_FILE = os.getenv(
    "REMOTE_DEMO_POSITIONS_FILE",
    os.path.join(".demo_runs", "telegram_demo_positions.json"),
)
REMOTE_DEMO_TOKEN_BUDGET = _env_int("REMOTE_DEMO_TOKEN_BUDGET", 200000)
REMOTE_DEMO_SIM_ROUNDS = _env_int("REMOTE_DEMO_SIM_ROUNDS", 1)

# ─── Trading Parameters ───────────────────────────────────────────────────────
MIN_EV = _env_float("MIN_EV", 0.10)
MAX_SPREAD = _env_float("MAX_SPREAD", 0.03)
MIN_VOLUME = _env_float("MIN_VOLUME", 500.0)
MIN_ENTRY_PRICE = _env_float("MIN_ENTRY_PRICE", 0.03)
MAX_ENTRY_PRICE = _env_float("MAX_ENTRY_PRICE", 0.95)
MIN_PROB_EDGE = _env_float("MIN_PROB_EDGE", 0.05)
MIN_ORDERBOOK_DEPTH_USD = _env_float("MIN_ORDERBOOK_DEPTH_USD", 1.0)
MAX_ORDERBOOK_SLIPPAGE = _env_float("MAX_ORDERBOOK_SLIPPAGE", 0.02)
MAX_POSITIONS_PER_CITY_DATE = _env_int("MAX_POSITIONS_PER_CITY_DATE", 1)
MAX_EXPOSURE_PER_CITY_DATE_USD = _env_float("MAX_EXPOSURE_PER_CITY_DATE_USD", 2.0)
MAX_OPEN_POSITIONS = _env_int("MAX_OPEN_POSITIONS", 20)
MAX_TOTAL_DEPLOYED_USD = _env_float("MAX_TOTAL_DEPLOYED_USD", 100.0)
MAX_PORTFOLIO_HEAT_USD = _env_float("MAX_PORTFOLIO_HEAT_USD", 20.0)
MAX_EXPOSURE_PER_TARGET_DATE_USD = _env_float("MAX_EXPOSURE_PER_TARGET_DATE_USD", 20.0)
MAX_DAILY_LOSS_USD = _env_float("MAX_DAILY_LOSS_USD", 25.0)
MAX_DRAWDOWN_USD = _env_float("MAX_DRAWDOWN_USD", 50.0)
MAX_DAILY_TRADES = _env_int("MAX_DAILY_TRADES", 20)
MIN_REWARD_RISK_RATIO = _env_float("MIN_REWARD_RISK_RATIO", 1.5)
MIN_HOURS_TO_RESOLUTION = _env_float("MIN_HOURS_TO_RESOLUTION", 2.0)
MAX_HOURS_TO_RESOLUTION = _env_float("MAX_HOURS_TO_RESOLUTION", 72.0)
KELLY_FRACTION = _env_float("KELLY_FRACTION", 0.25)
MAX_TRADE_SIZE_USD = _env_float("MAX_TRADE_SIZE_USD", 20.0)
STOP_LOSS_PCT = _env_float("STOP_LOSS_PCT", 0.20)
TRAILING_STOP_TRIGGER = _env_float("TRAILING_STOP_TRIGGER", 0.20)

# ─── Simulation Parameters ────────────────────────────────────────────────────
SIM_ROUNDS = _env_int("SIM_ROUNDS", 3)
MAX_AGENTS_PER_SIM = 4       # weather analyst agents per city
CONSENSUS_THRESHOLD = _env_float("CONSENSUS_THRESHOLD", 0.65)
HIGH_SPREAD_THRESHOLD_F = _env_float("HIGH_SPREAD_THRESHOLD_F", 8.0)

# ─── Execution / Parallelism ─────────────────────────────────────────────────
# Max cities processed concurrently. Higher = faster cycle but more API load.
MAX_PARALLEL_CITIES = _env_int("MAX_PARALLEL_CITIES", 4)
# Max parallel order-book HTTP requests within a single city's market scan.
MAX_PARALLEL_ORDERBOOKS = _env_int("MAX_PARALLEL_ORDERBOOKS", 8)
# Hours-to-resolution threshold below which IOC orders are used instead of GTC.
IOC_URGENCY_HOURS = _env_float("IOC_URGENCY_HOURS", 6.0)
# Order submission retries (exponential backoff: 1s, 2s, 4s).
ORDER_RETRY_MAX = _env_int("ORDER_RETRY_MAX", 3)
...
SCENARIO_THRESHOLD_F = _env_float("SCENARIO_THRESHOLD_F", 0.0)
...
MAX_DEBATE_TRANSCRIPTS = _env_int("MAX_DEBATE_TRANSCRIPTS", 200)
...
DECISION_MIN_LIVE_FEATURE_QUALITY = _env_float("DECISION_MIN_LIVE_FEATURE_QUALITY", 0.60)
DECISION_LOW_DATA_RISK_MULTIPLIER = _env_float("DECISION_LOW_DATA_RISK_MULTIPLIER", 0.50)
DECISION_MAX_HOLDER_CONCENTRATION = _env_float("DECISION_MAX_HOLDER_CONCENTRATION", 0.65)
DECISION_CONTRA_FLOW_THRESHOLD = _env_float("DECISION_CONTRA_FLOW_THRESHOLD", 0.35)
DECISION_MAX_VOLATILITY_24H = _env_float("DECISION_MAX_VOLATILITY_24H", 0.20)
USE_WEATHER_NORMALS = (
...
UPDATE_INTERVAL_SECONDS = _env_int("UPDATE_INTERVAL_SECONDS", 3600)
